...
def get_conditions(conditions_file_path):
    ...
    data = pd.read_excel(conditions_file_path)
    connection = sqlite3.connect(DATABASE_PATH)
...
def get_psychologists(psychologists_file_path):
    ...
    data = pd.read_excel(psychologists_file_path)
    connection = sqlite3.connect(DATABASE_PATH)
    cursor = connection.cursor()
...
def get_candidates(candidates_file_path):
    ...
    data = pd.read_excel(candidates_file_path)
...
def get_cond_of_cand(path_to_candidates_conditions_file):
    ...
    data: pd.DataFrame = pd.read_excel(path_to_candidates_conditions_file, header=None, index_col=0).transpose()
...
def get_working_time(path_to_working_hours_file):
    ...
    data: pd.DataFrame = pd.read_excel(path_to_working_hours_file)
    days = []
...
def get_candidates_available_hours(available_hours_file):
    ...
    data: pd.DataFrame = pd.read_excel(available_hours_file, header=None, index_col=0).transpose()
