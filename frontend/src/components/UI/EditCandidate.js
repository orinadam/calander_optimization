...
const EditCandidate = (props) => {
  ...
  const current = {
    day: candidate.day,
    hour: candidate.hour,
    psychologist: candidate.psychologist,
  };
  const [selected, setSelected] = useState(
    `${current.psychologist}-${current.day}-${current.hour}`
  );
  ...
  const showOptions = options.map((option) => {
    return (
      <option
        value={`${option.psychologist}-${option.day}-${option.hour}`}
        ...
        <Modal.Body>
          <Form onSubmit={handleSubmitUpdate}>
            ...
            >
              ...
              <option
                value={`${current.psychologist}-${current.day}-${current.hour}`}
              >{`${current.psychologist} - ${current.day} - ${current.hour}`}</option>
              {showOptions}
            </Form.Select>
            <br />
            <div>
